...
def adversarial_signal(rng, true_risk, false_positive_rate):
    ...
    false_pos = safe & (
        rng.random(AGENTS) < false_positive_rate
    )

    false_neg = risky & (
        rng.random(AGENTS) < FALSE_NEGATIVE_RATE
    )
...
def apply_correction(state, mask, rng):
    ...
    corrected[mask] = (
        corrected[mask]
        - CORRECTION_GAIN * corrected[mask]
        + noise[mask]
    )
...
summary.to_csv(
    f"{OUTDIR}/noise_sweep_summary.csv"
)
